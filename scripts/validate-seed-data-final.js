...
/**
 * Final Seed Data Validation Script
 * Validates Dragon Quest story data for completeness and consistency
 * 
 * Usage: node scripts/validate-seed-data-final.js
 */

const fs = require('fs');
const path = require('path');

class FinalSeedDataValidator {
    constructor() {
    ...
    parseSeedFile() {
        ...
        try {
            ...
            // Parse events
            this.parseTable(content, 'story_events', this.data.events);
            // Parse interactions  
            this.parseTable(content, 'event_interactions', this.data.interactions);
            // Parse outcomes
            this.parseTable(content, 'event_outcomes', this.data.outcomes);
    ...
    parseTable(content, tableName, targetArray) {
        ...
        matches.forEach(match => {
            ...
            // Split into rows, handling comments and empty lines
            const rows = valuesContent
                .split('),\n(')
                .map(row => row.trim())
                .filter(row => row && !row.startsWith('--'))
                .map(row => row.replace(/,\s*$/, '')); // Remove trailing commas
            
            rows.forEach(rowStr => {
                const row = this.parseRowValues(rowStr, tableName);
                if (row) {
    ...
    parseRowValues(valueStr, tableName) {
        try {
            // Clean up the value string
            valueStr = valueStr.trim();
            
            // Split by commas but respect quoted strings and handle escaped quotes
            const values = [];
            let current = '';
            let inQuotes = false;
            let i = 0;
            ...
            const row = {};
            const currentColumns = columns[tableName];
    ...
    validate() {
        ...
        this.data.outcomes.forEach(outcome => {
            ...
            let hasUnlockEvents = false;
            
            if (outcome.effects && outcome.effects !== 'null') {
                try {
                    const effects = JSON.parse(outcome.effects);
                    hasUnlockEvents = effects.unlock_events && effects.unlock_events.length > 0;
                } catch (e) {
                    // Invalid JSON, will be caught by another rule
                }
            }
    ...
    generateWarnings() {
        ...
        this.data.events.forEach(event => {
            ...
            const hasProgression = eventOutcomes.some(outcome => {
                let hasNextEvent = outcome.next_event_id && outcome.next_event_id !== 'null';
                let hasUnlockEvents = false;
            ...
            if (!hasProgression && eventOutcomes.length > 0) {
                this.warnings.push(
...
if (require.main === module) {
    const validator = new FinalSeedDataValidator();
    validator.run();
}

module.exports = FinalSeedDataValidator;
